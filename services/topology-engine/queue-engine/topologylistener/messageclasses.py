...
#            return True
    ...
    def mark_network_offline(self):
        ...
        queue_switch_nodes = (
            'MATCH (sw:switch)\n'
            'SET sw.status=\'{status}\'').format(status=SW_STATUS_UNKNOWN)
        queue_isl_nodes = (
            'MATCH (:switch)-[l:isl]->(:switch)\n'
            'SET l.status=\'{status}\'').format(status=ISL_STATUS_UNKNOWN)

        with neo4j_update_lock, graph.gebin() as tx:
            for q in queue_switch_nodes, queue_isl_nodes:
                tx.run(q)

        logger.info('Network topology is in UNKNOWN state')

        # propagate status into cache and flow topologies
        message_utils.send_cache_message(self.payload, self.correlation_id)
        message_utils.send_to_topic(
